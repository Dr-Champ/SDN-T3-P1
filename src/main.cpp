    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          struct CarLocation loc;
          loc.car_x = j[1]["x"];
          loc.car_y = j[1]["y"];
          loc.car_s = j[1]["s"];
          loc.car_d = j[1]["d"];
          loc.car_yaw = j[1]["yaw"];
          loc.car_speed = j[1]["speed"];

          // Previous path data given to the Planner
          struct PreviousLocation prevLoc;
          auto previous_path_x = j[1]["previous_path_x"];
          for (auto x: previous_path_x)
            prevLoc.previous_path_x.push_back(x);
          
          auto previous_path_y = j[1]["previous_path_y"];
          for (auto y: previous_path_y)
            prevLoc.previous_path_y.push_back(y);

          // Previous path's end s and d values 
          prevLoc.end_path_s = j[1]["end_path_s"];
          prevLoc.end_path_d = j[1]["end_path_d"];
            ...
            PathPlanner ppn;
            ppn.computePath(next_x_vals, next_y_vals, map, loc, prevLoc);
